...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   preguntar(Preg,Resp):-new(Di,dialog('Colsultar Datos:')),
                        new(L2,label(texto,'RESPONDE CON LA VERDAD, TODO LO QUE DIGAS SERA USADO
						EN TU CONTRA...')),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  interfaz_principal:-new(@main,dialog('SISTEMA MULTIVERGAS EXPERTO',
        size(1000,3000))),
        new(@texto, label(nombre,'SE VA A MORIR DE...',font('times','roman',18))),
        new(@resp1, label(nombre,'',font('times','roman',32))),
        new(@lblExp1, label(nombre,'',font('times','roman',14))),
        new(@lblExp2, label(nombre,'',font('times','roman',14))),
        new(@salir,button('A LA VERGUI',and(message(@main,destroy),message(@main,free)))),
        new(@boton, button('NO TENGAS MIEDO',message(@prolog, botones))),

        new(@btntratamiento,button('NO TIENE CURA, SABES PORQUE?')),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  crea_interfaz_inicio:- new(@interfaz,dialog('ESTE ES MI PRIMERA VENTANA GRÁFICA lol XD :* ----',
  size(1000,1000))),

  mostrar_imagen(@interfaz, portada),

  new(BotonComenzar,button('VAMOS ADELANTE CAMPEÓN',and(message(@prolog,interfaz_principal) ,
  and(message(@interfaz,destroy),message(@interfaz,free)) ))),
  new(BotonSalir,button('A LA VERGUI',and(message(@interfaz,destroy),message(@interfaz,free)))),
