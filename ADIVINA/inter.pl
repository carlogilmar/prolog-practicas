...
*/
 ...
 resource(megan, image, image('trat_hidropesia.jpg')).
 resource(elisha, image, image('trat_vejiga.jpg')).
 resource(johnny, image, image('trat_ich.jpg')).
 resource(leonardo, image, image('trat_estres.jpg')).
 resource(daisy, image, image('trat_hexamita.jpg')).
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
  botones:-borrado,
                ...
                send(@texto, selection('ADIVINA QUIEN DICE QUE ES:')),
                send(@resp1, selection(Enfermedad)),
                new(@boton, button('Empezar a adivinar',
                message(@prolog, botones)
                )),

                new(@btntratamiento,button('Imagen de la Persona',
  ...
  mostrar_tratamiento(X):-new(@tratam, dialog('Personaje Adivinado por el Sistema')),
                          send(@tratam, append, label(nombre, ' Debe ser... ')),
...
tratamiento(X):- send(@lblExp1,selection('Segun la información la persona es:')),
                 mostrar_imagen_tratamiento(@tratam,X).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  

   preguntar(Preg,Resp):-new(Di,dialog('Adivinando:')),
                        new(L2,label(texto,'Responde con un si o no')),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  interfaz_principal:-new(@main,dialog('Sistema Experto Que Adivina La Persona',
        size(1000,1000))),
        new(@texto, label(nombre,'La persona debe ser:',font('times','roman',18))),
        new(@resp1, label(nombre,'',font('times','roman',22))),
        new(@lblExp1, label(nombre,'',font('times','roman',14))),
        new(@lblExp2, label(nombre,'',font('times','roman',14))),
        new(@salir,button('Vamonos',and(message(@main,destroy),message(@main,free)))),
        new(@boton, button('Adivinar',message(@prolog, botones))),

        new(@btntratamiento,button('Ver Imagen')),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  crea_interfaz_inicio:- new(@interfaz,dialog('SISTEMA ADIVINADOR',
  size(1000,1000))),

  mostrar_imagen(@interfaz, portada),

  new(BotonComenzar,button('¿ELEGISTE? A ADIVINAR',and(message(@prolog,interfaz_principal) ,
